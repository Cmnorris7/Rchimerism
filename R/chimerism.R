...
clean_pre_file <- function(file){
  ...
  #     gui = .GUI)
  #   peak_table <- fread(file$res, sep = '\t', header=TRUE, na.strings=c("","NA"))
  # } else{
...
# Single donor -- Take in 3 files and remove unwanted peaks from sample file
...
# Double donor -- Take in 4 files and remove unwanted peaks from sample file
...
multi_donor <- function(pre_don1=NULL,pre_don2=NULL,recip=NULL,post=NULL){
  
  d1data <- clean_pre_file(pre_don1)
  d2data <- clean_pre_file(pre_don2)
  rdata <- clean_pre_file(recip)
  sdata <- get_informative_marks_dd(d1data, d2data, rdata, post)
...
# writes peak reports into peak_report table -- probably not used?
...
arg_list = list(
  make_option("--don1", type="character", default=NULL, 
    help="pre-transplant donor peak report file path", metavar="character"),
  make_option("--don2", type="character", default=NULL, 
    help="pre-transplant donor peak report file path(2nd donor)", metavar="character"),
  make_option("--recip", type="character", default=NULL,
    help="pre-transplant recipient peak report file path", metavar="character"),
  make_option("--post", type="character", default=NULL,
    help="post-transplant peak report file path", metavar="character"),
  make_option("--workflow", type="character", default=NULL,
    help=
    "specify which workflow needs to be run:
      \t\t[--workflow pre_don] -- Analyze pre-transplant donor sample & save to database. Requires --don1 and/or --don2 files
      \t\t[--workflow pre_recip] -- Analyze pre-transplant recipient sample & save to database. Requires --recip file.
      \t\t[--workflow sd] -- Analyze post-transplant for single donor. Requires --don1, --recip, --post
      \t\t[--workflow dd] -- Analyze post-transplant for double donor. Requires --don1, --don2, --recip, --post", metavar="character")
)
...
} else if (opt$workflow == 'pre_recip'){
